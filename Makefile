...
.PHONY: install run-network studio milvus-lite clean test test-fast cov cov-html verify-tests quick-verify lint format ci operator
...
operator: install
    $(VENV_BIN)/python -m tools.operator
...
verify-tests:
    $(VENV_BIN)/python scripts/quick_verify.py
