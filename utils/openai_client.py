...
class ChatAPIConfig:
    ...
    def from_env(cls, *, load_env: bool = True) -> "ChatAPIConfig":
        ...
        if base_url_raw is not None:
            base_url = base_url_raw.strip() if isinstance(base_url_raw, str) else str(base_url_raw)
        
        model_raw = _get_env_value("CHAT_API_MODEL", ("OPENAI_MODEL",))
        ...
        provider_raw = _get_env_value("CHAT_API_PROVIDER")
        if provider_raw is None:
            provider_candidate = "openai"
        elif isinstance(provider_raw, str):
            provider_candidate = provider_raw.strip()
        else:
            provider_candidate = str(provider_raw)
        ...
        try:
            ...
            logger.warning(f"Unknown provider '{provider_candidate}', defaulting to 'openai'")
...
class EmbeddingAPIConfig:
    ...
    def from_env(cls, *, load_env: bool = True) -> "EmbeddingAPIConfig":
        ...
        model_raw = _get_env_value("EMBEDDING_API_MODEL", ("EMBEDDING_MODEL",))
        if model_raw is None:
        ...
        provider_raw = _get_env_value("EMBEDDING_API_PROVIDER")
        if provider_raw is None:
            provider_candidate = "openai"
        elif isinstance(provider_raw, str):
            provider_candidate = provider_raw.strip()
